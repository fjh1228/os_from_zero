...
extern intr_handler intr_entry_table[IDT_CNT];
struct gate_desc idt[IDT_CNT];          //中断门描述符（结构体），不是中断描述符表，一个中断门可以在中断描述符表中存在很多的
intr_handler idt_table[IDT_CNT];
char* intr_name[IDT_CNT];


/***************************************补全中断描述符表************************************/
static void gate_desc_init(struct gate_desc* p_gdesc, uint8_t attribute, intr_handler intr_func){
        p_gdesc->low_16_offset = (0x0000FFFF) & (uint32_t)(intr_func);
        p_gdesc->selector = SELECOTR_K_CODE;
        p_gdesc->cnt = 0;
        p_gdesc->attr = attribute;
        p_gdesc->high_16_offset =((0xFFFF0000) & (uint32_t)(intr_func))>>16;
}
...
static void idt_desc_init(){
    ...
    int i;
    for(i = 0; i < IDT_CNT; i++){
        gate_desc_init(&idt[i], IDT_DESC_ATTR_DPL0,intr_entry_table[i]);
        if (i == 0){
